...
const Schedule = {
    ...
    readList: async function (uid, date) {
        try {
            const [results] = await db.query(
                'SELECT sid, date, start_at, end_at, color_hex, title, detail, location, created_at, edited_at, done, done_at FROM tb_schedules WHERE uid = ? AND date = ?',
                [uid, date],
            );
            return results;
        } catch (error) {
    ...
    read: async function (sid, uid) {
        try {
            const [results] = await db.query(
                'SELECT sid, date, start_at, end_at, color_hex, title, detail, location, created_at, edited_at, done, done_at FROM tb_schedules WHERE sid = ? AND uid = ? LIMIT 1',
                [sid, uid],
            );
            if (results.length === 0) {
