...
const DailyTodo = {
    serialization: async function (results) {
        return await results.reduce((acc, {
            label_id, label_origin_id, label_title, label_color_hex, label_date, id, title, status, created_at, updated_at,
        }) => {
    ...
    create: async function (todo) {
        try {
            const [result] = await db.query(
                'INSERT INTO tb_daily_todo (daily_label_id, title) SELECT id, ? FROM tb_daily_todo_labels WHERE id = ? AND uid = ?',
                [todo.title, todo.dailyLabelId, todo.uid],
            );
    ...
    updateTitle: async function (todo) {
        try {
            const [results] = await db.query(
                'UPDATE tb_daily_todo tdt JOIN tb_daily_todo_labels tdtl ON tdt.daily_label_id = tdtl.id SET tdt.title = ? WHERE tdt.id = ? AND tdtl.uid = ?',
                [todo.title, todo.id, todo.uid],
            );
            if (results.affectedRows === 0) {
                throw {status: 404, message: "존재하지 않는 항목입니다"};
            }
    ...
    read: async function (id, uid) {
        try {
            const [results] = await db.query(
                'SELECT tdtl.id AS "label_id", tdtl.label_id AS "label_origin_id", ttl.title AS "label_title", ttl.color_hex AS "label_color_hex", tdtl.date AS "label_date", tdt.id, tdt.title, tdt.status, tdt.created_at, tdt.updated_at FROM tb_daily_todo tdt JOIN tb_daily_todo_labels tdtl ON tdt.daily_label_id = tdtl.id JOIN tb_todo_labels ttl ON tdtl.label_id = ttl.id WHERE tdt.id = ? AND tdtl.uid = ? LIMIT 1',
                [id, uid],
            );
            if (results.length === 0) {
    ...
    readAllByDate: async function (date, uid) {
        try {
            const [results] = await db.query(
                    'SELECT tdtl.id AS "label_id", tdtl.label_id AS "label_origin_id", ttl.title AS "label_title", ttl.color_hex AS "label_color_hex", tdtl.date AS "label_date", tdt.id, tdt.title, tdt.status, tdt.created_at, tdt.updated_at FROM tb_daily_todo tdt JOIN tb_daily_todo_labels tdtl ON tdt.daily_label_id = tdtl.id JOIN tb_todo_labels ttl ON tdtl.label_id = ttl.id WHERE tdtl.date = ? AND tdtl.uid = ?',
                [date, uid],
            );
            if (results.length === 0) {
    ...
    readAllByDailyLabel: async function (dailyLabelId, uid) {
        try {
            const [results] = await db.query(
                'SELECT tdtl.id AS "label_id", tdtl.label_id AS "label_origin_id", ttl.title AS "label_title", ttl.color_hex AS "label_color_hex", tdtl.date AS "label_date", tdt.id, tdt.title, tdt.status, tdt.created_at, tdt.updated_at FROM tb_daily_todo tdt JOIN tb_daily_todo_labels tdtl ON tdt.daily_label_id = tdtl.id JOIN tb_todo_labels ttl ON tdtl.label_id = ttl.id WHERE tdtl.id = ? AND tdtl.uid = ?',
                [dailyLabelId, uid],
            );
            if (results.length === 0) {
